...
function RoleChip({ role }: { role: UserRole }) {
  ...
  return <Chip label={roleLabels[role]} size="small" sx={{ backgroundColor: bg, color, fontSize: 12, fontWeight: 700, height: 22 }} />
}
...
function UserDialog({ open, user, onClose, onSave }: DialogProps) {
  ...
  const validateSenha = (s: string) => {
    if (!isEditing && s.length === 0) { setSenhaErro('Senha é obrigatória'); return false }
    if (s.length > 0 && s.length < 8) { setSenhaErro('Mínimo 8 caracteres'); return false }
    if (s.length > 0 && !/(?=.*[a-zA-Z])(?=.*[0-9])/.test(s)) { setSenhaErro('Deve conter letras e números'); return false }
    setSenhaErro(''); return true
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <IconButton size="small" onClick={onClose} aria-label="Fechar dialog"><CloseIcon /></IconButton>
      </DialogTitle>
      <Divider />
      <DialogContent sx={{ pt: 2.5 }}>
        {/* Informações básicas */}
        <Typography variant="caption" sx={{ fontSize: 12, fontWeight: 700, color: '#902B29', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', mb: 1.5 }}>
          Informações Básicas
        </Typography>
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            <TextField
              fullWidth size="small" label={isEditing ? 'Nova Senha' : 'Senha *'} type="password"
              value={senha} onChange={e => { setSenha(e.target.value); if (e.target.value) validateSenha(e.target.value) }}
              error={!!senhaErro} helperText={senhaErro || (isEditing && !senha ? 'Deixe em branco para manter a atual' : '')}
              required={!isEditing}
            />
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
        ...
        {/* Permissões */}
        <Divider sx={{ mb: 2 }} />
        <Typography variant="caption" sx={{ fontSize: 12, fontWeight: 700, color: '#902B29', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', mb: 1 }}>
...
export default function UsuariosPage() {
  ...
  const openEdit = (u: SystemUser) => { setSelected(u); setDialogOpen(true) }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={openCreate} sx={{ mt: 0.5, minHeight: 44 }}>
          Novo Usuário
        </Button>
      </Box>
        ...
        <Table size="small">
          ...
          <TableBody>
            {filtered.length === 0 ? (
              ...
              <TableRow key={u.id} sx={{ cursor: 'default', transition: 'background-color 0.15s ease', '&:hover': { backgroundColor: 'rgba(144,43,41,0.03)' } }}>
                <TableCell>
                ...
                <TableCell>
                  <Typography variant="body2" sx={{ fontSize: 12, color: 'text.secondary' }}>{u.email}</Typography>
                </TableCell>
